...
# Warning: To avoid errors Modeler and Tester configuration will share common variables. If modeler is used first and tester used second to test there should be no errors'''
...
stop_loss = 0  # A Stop Loss will be place at - the % value defined here every time a buy order is placed. True Range (TR) indicator will be used instead of percentage by setting stop_loss = 'tr'
...
md_start_date = '2022-01-01 0:00:00'  # By setting a start date the program will select a portion of the dataset from that date to train the model. (Date should exist in the dataset to avoid errors). This value is also used to calculate the number of days of the model imprinted in the file
...
real_rf_ret = 0.0001428  # Based on the current 3 month T-bill that yield 5.35% per year (0.0001428 daily)
...
mul = 1  # When creating the lists of the indicators random parameters this variable can be used to scale them (2x, 3x, etc)
...
seg_n = 8  # NºSegments, how many subsequent segments shall be tested. if the segment length equals a month in time and this number is 6 it will test 6 months starting from the specified date (md_start_date). Make sure not to go beyond the last date of the imported testing dataset to avoid errors.
chat_id = 'Fill with Telegram Chat ID number'
asset_precision = 5
o_type = "MARKET"  #Order type ("LIMIT" or "MARKET") . Sets to trade on market (Market fee) or on a limit (Maker fee) for lower fees.
sample_freq = '1m'
since = 10
